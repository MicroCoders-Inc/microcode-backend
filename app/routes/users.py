...
@users_bp.route("/users/<int:user_id>/owned-courses/<int:course_id>", methods=["DELETE"])
...
def get_favourite_courses(user_id):
    ...
    if expand:
        ...
        return jsonify({
            "user_id": user.id,
            "favourite_courses": courses
        }), 200
    
    return jsonify({
        "user_id": user.id,
        "favourite_courses": user.favourite_courses or []
    }), 200
...
@users_bp.route("/users/<int:user_id>/favourite-courses/<int:course_id>", methods=["DELETE"])
...
def get_saved_blogs(user_id):
    ...
    return jsonify({
        "user_id": user.id,
        "saved_blogs": user.saved_blogs or []
    }), 200
